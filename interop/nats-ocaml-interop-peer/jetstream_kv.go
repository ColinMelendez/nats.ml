...
func checkKeyValueStatus(label string, status natsjetstream.KeyValueStatus, bucket string, values uint64) error {
...
func runJetStreamKeyValuePeer(config options) error {
	...
	if err := checkKeyValueStatus("initial Key-Value status", status, config.bucket, 0); err != nil {
		return err
	}
