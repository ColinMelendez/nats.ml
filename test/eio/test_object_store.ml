...
let trace_consumer_name ~trace ~occurrence =
  let value = Buffer.contents trace in
  let marker = "PUB $JS.API.CONSUMER.CREATE.OBJ_assets." in
  let marker_length = String.length marker in
  let limit = String.length value - marker_length in
  let position = ref 0 in
  let seen = ref 0 in
  ...
  | None ->
      fail
        (Format.asprintf
           "trace did not contain consumer-create occurrence %d; trace:\n%s"
           occurrence value)
  | Some start ->
      let finish = ref start in
      while
        !finish < String.length value
        && not (Char.equal (String.get value !finish) ' ')
      do
        incr finish
      done;
      if Int.equal !finish (String.length value) then
        fail
          (Format.asprintf "consumer-create occurrence %d was unterminated"
             occurrence)
...
let config () =
  ...
  match
    Nats_eio.Object_store.Config.v ~bucket:"assets" ~description:"media"
      ~ttl:Mtime.Span.(1 * s)
      ~max_bytes:4096L ~storage:Nats_eio.Object_store.Config.Memory ~replicas:3
      ~placement ~compression:Nats_eio.Object_store.Config.S2
      ~metadata:[ ("owner", "object-test") ]
      ()
  with
  | Ok value -> value
  | Error error ->
      fail (Format.asprintf "%a" Nats_eio.Object_store.Error.pp_config error)

let object_name value =
  match Nats_eio.Object_store.Name.of_string value with
  | Ok value -> value
  | Error error ->
      fail (Format.asprintf "%a" Nats_eio.Object_store.Error.pp_name error)
...
let object_meta_description ~description ~name ~chunk_size =
  match
    Nats_eio.Object_store.Meta.v ~name:(object_name name) ~description
      ~chunk_size ()
  with
  | Ok value -> value
  | Error error ->
...
let object_ordered_create_wire ~sid ~name () =
  let payload =
    Format.asprintf
...
let object_watch_delivery_wire ~sid ~stream_sequence ~consumer_sequence ~pending
    ~name ~nuid ~size ~chunks ~digest ?(deleted = false) () =
  let encoded_name =
    Base64.encode_string ~alphabet:Base64.uri_safe_alphabet name
  in
  let reply_to =
    Format.asprintf "$JS.ACK.OBJ_assets.watch-1.1.%Ld.%Ld.0.%Ld" stream_sequence
      consumer_sequence pending
  in
  let payload =
    Format.asprintf
      ...
      name nuid size chunks digest
      (if deleted then "true" else "false")
  in
  let message =
    Nats.Message.v
      ~subject:(Nats.Subject.literal ("$O.assets.M." ^ encoded_name))
      ~reply_to:(Nats.Subject.literal reply_to)
      payload
  in
  operation_wire (Nats.Op.Hmsg { sid; message; status = None })
  ...
  run "nats-eio-object-store"
    [
      test "configuration and metadata validate external values" (fun () ->
          ...
          | Ok value ->
              equal string "assets" (Nats_eio.Object_store.Config.bucket value);
              equal (option string) None
                (Nats_eio.Object_store.Config.description value)
          | Error error ->
              fail
                (Format.asprintf "%a" Nats_eio.Object_store.Error.pp_config
                   error));
            ...
            | Error error ->
                fail
                  (Format.asprintf "%a" Nats_eio.Object_store.Error.pp_name
                     error)
          in
          match Nats_eio.Object_store.Meta.v ~name ~chunk_size:0 () with
      ...
      test "create and status project the object-store stream contract"
        (fun () ->
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              require_trace ~trace
                ~needle:"placement\\\":{\\\"cluster\\\":\\\"objects\\\"";
              require_trace ~trace ~needle:"compression\\\":\\\"s2";
              require_trace ~trace
                ~needle:"metadata\\\":{\\\"owner\\\":\\\"object-test\\\"}";
              ...
              let status =
                ...
                | Error error ->
                    fail
                      (Format.asprintf "%a" Nats_eio.Object_store.Error.pp error)
              in
              equal string "assets" (Nats_eio.Object_store.Status.bucket status);
              ...
              equal int64 9L (Nats_eio.Object_store.Status.last_sequence status);
              equal int64 4096L
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              let placement =
                ...
                | Error error ->
                    fail
                      (Format.asprintf "%a" Nats_eio.Jetstream.Error.pp_config
                         error)
              in
              let config =
                match
                  Nats_eio.Object_store.Config.v ~bucket:"assets"
                    ...
                    ~ttl:Mtime.Span.(2 * s)
                    ~max_bytes:8192L ~storage:Nats_eio.Object_store.Config.File
              ...
              if
                contains_substring ~needle:"PUB $O.assets.M."
                  (Buffer.contents trace)
              ...
              expect_ok (Nats_eio.Connection.close connection);
              Eio.Promise.resolve hold_u (Error End_of_file)));
      test "metadata update preserves content identity without uploading chunks"
        (fun () ->
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              if
                contains_substring ~needle:"PUB $O.assets.C."
                  (Buffer.contents trace)
              then fail "metadata update uploaded a chunk";
              if
                contains_substring ~needle:"STREAM.PURGE.OBJ_assets"
                  (Buffer.contents trace)
              then fail "metadata update purged content"));
      test "metadata update can rename before purging the old subject"
        (fun () ->
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              Eio.Fiber.fork ~sw (fun () ->
                  Eio.Promise.resolve result_u
                    (Nats_eio.Object_store.update
                       ~name:(object_name "images/cat.png")
                       bucket
              ...
              wait_for_trace_count ~trace ~needle:"STREAM.PURGE.OBJ_assets"
                ~count:1;
              require_trace ~trace ~needle:"$O.assets.M.aW1hZ2VzL2NhdC5wbmc=";
      ...
      test "put_link commits a metadata-only object link" (fun () ->
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              Eio.Promise.resolve target_info_u
                (Ok
                   (object_info_wire ~sid:2 ~name:"images/base.png"
                      ~nuid:"base-nuid" ~size:2L ~chunks:1L
                      ~digest:"SHA-256=base" ~chunk_size:2));
              wait_for_trace_count ~trace ~needle:"PUB $O.assets.M." ~count:1;
              require_trace ~trace
                ~needle:
                  "link\\\":{\\\"bucket\\\":\\\"assets\\\",\\\"name\\\":\\\"images/base.png\\\"";
              require_trace ~trace ~needle:"chunks\\\":0";
              if
                contains_substring ~needle:"max_chunk_size\\\":131072"
                  (Buffer.contents trace)
              then fail "put_link invented a chunk-size option";
              Eio.Promise.resolve metadata_u
                (Ok
                   (response_wire_with_sid ~sid:3
                      {|{"stream":"OBJ_assets","seq":21}|}));
              wait_for_trace_count ~trace
                ~needle:"PUB $JS.API.DIRECT.GET.OBJ_assets" ~count:3;
              Eio.Promise.resolve final_info_u
                (Ok
                   (object_link_info_wire ~sid:4 ~name:"images/cat.png"
              ...
              | Some link ->
                  equal string "assets" (Nats_eio.Object_store.Link.bucket link);
      ...
      test "put_link refuses to replace an ordinary object" (fun () ->
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              Eio.Promise.resolve hold_u (Error End_of_file)));
      test "put preserves reader slices and commits interoperable metadata"
        (fun () ->
          let initial_info, initial_info_u = Eio.Promise.create () in
                ...
                Eio.Fiber.fork ~sw (fun () ->
                    Eio.Promise.resolve result_u
                      (Nats_eio.Object_store.put
                         ...
                         bucket meta
                         (Bytesrw.Bytes.Reader.of_string ~slice_length:3 "abcde")));
                wait_for_trace_count ~trace
                  ~needle:"PUB $JS.API.DIRECT.GET.OBJ_assets" ~count:1;
                Eio.Promise.resolve initial_info_u
                  (Ok (response_wire_with_sid ~sid:1 ""));
      ...
      test "get follows object links before streaming target chunks" (fun () ->
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              equal string "target-nuid" (Nats_eio.Object_store.Info.nuid info);
              require_trace ~trace ~needle:"$O.assets.C.target-nuid"));
      test "watch drains retained metadata and emits an initial marker"
        (fun () ->
          let create_response, create_response_u = Eio.Promise.create () in
          let info_response, info_response_u = Eio.Promise.create () in
          let first_delivery, first_delivery_u = Eio.Promise.create () in
        ...
        (fun () ->
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              wait_for_trace_count ~trace ~needle:"STREAM.INFO.OBJ_assets"
                ~count:1;
              Eio.Promise.resolve first_info_u
              ...
              Eio.Promise.resolve second_info_u
                (Ok (stream_info_response ~sid:2 ~bucket:"assets" ()));
              wait_for_trace_count ~trace ~needle:"STREAM.UPDATE.OBJ_assets"
                ~count:1;
              require_trace ~trace ~needle:"sealed\\\":true";
              Eio.Promise.resolve update_response_u
                (Ok
                   (stream_info_response ~sid:3 ~bucket:"assets" ~sealed:true ()));
              wait_for_trace_count ~trace ~needle:"STREAM.INFO.OBJ_assets"
                ~count:3;
              Eio.Promise.resolve third_info_u
                (Ok
                   (stream_info_response ~sid:4 ~bucket:"assets" ~sealed:true ()));
              let status =
                match Eio.Promise.await result with
                | Ok status -> status
                | Error error ->
      ...
      test "replacement commits new metadata before purging old chunks"
        (fun () ->
          let initial_info, initial_info_u = Eio.Promise.create () in
          ...
          with_connection_traced
            ...
            (fun ~sw ~trace connection ->
              ...
              Eio.Promise.resolve initial_info_u
                (Ok
                   (object_info_wire ~sid:1 ~name:"images/cat.png"
                      ~nuid:"old-nuid" ~size:3L ~chunks:1L ~digest:"SHA-256=old"
                      ~chunk_size:2));
              wait_for_trace_count ~trace ~needle:"PUB $O.assets.C." ~count:1;
                ...
                | Error error ->
                    fail
                      (Format.asprintf "%a\ntrace:\n%s"
                         ...
                         (Buffer.contents trace))
              in
              equal string "new-nuid" (Nats_eio.Object_store.Info.nuid info);
              require_trace ~trace ~needle:"Nats-Rollup"));
      test "delete publishes a tombstone before purging object chunks"
        (fun () ->
          let info_response, info_response_u = Eio.Promise.create () in
          let metadata, metadata_u = Eio.Promise.create () in
          let purge, purge_u = Eio.Promise.create () in
