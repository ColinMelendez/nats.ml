...
let expect_connected ~clock events =
  ignore
    (wait_for_event ~clock ~label:"connected" ~remaining:16
       (function
...
let expect_discovered info expected =
  ...
  List.iter
    (fun value ->
      if not (contains value urls) then
...
let run env =
  ...
  let signal = required "NATS_TEST_CLUSTER_SIGNAL" in
  let initial_servers =
    required "NATS_TEST_CLUSTER_INITIAL_SERVERS"
    |> String.split_on_char ','
    |> List.filter (fun value -> not (String.equal value ""))
  in
  let recovered_servers =
    required "NATS_TEST_CLUSTER_RECOVERED_SERVERS"
    |> String.split_on_char ','
    |> List.filter (fun value -> not (String.equal value ""))
  in
  let discovered =
    required "NATS_TEST_CLUSTER_DISCOVERED"
    |> String.split_on_char ','
    |> List.filter (fun value -> not (String.equal value ""))
  in
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      let filter = Nats.Subject.Filter.literal "ocaml.integration.cluster" in
      let subscription =
      ...
      | Some value when List.exists (String.equal value) recovered_servers -> ()
      | _ ->
...
let () =
  ...
  | Failure message ->
      prerr_endline ("server cluster failed: " ^ message);
      exit 1
  | error ->
      prerr_endline ("server cluster failed: " ^ Printexc.to_string error);
      exit 1
