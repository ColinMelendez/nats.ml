...
let run_durable_push ~sw ~timeout ~jetstream ~stream ~filter ~subject =
  ...
  let config =
    expect_jetstream_config_ok "durable push config"
      (Nats_eio.Jetstream.Consumer.Config.v ~durable_name:"OCAML_TEST_PUSH"
...
let run_owned_push ~sw ~timeout ~jetstream ~stream ~filter ~subject =
  ...
  Fun.protect
    ...
    (fun () ->
      ignore
        (publish ~timeout jetstream ~msg_id:"consumer-push-2" subject "push-two");
      let message =
      ...
      | Error error ->
          failf "owned push info after close: %s"
            (jetstream_error_message error))

let run_ordered ~sw ~clock ~timeout ~jetstream ~stream ~filter ~subject
    ~other_subject =
  ignore
    (publish ~timeout jetstream ~msg_id:"consumer-ordered-1" subject
       "ordered-one");
  ignore
    (publish ~timeout jetstream ~msg_id:"consumer-ordered-gap" other_subject
       "not-ordered");
  ignore
    (publish ~timeout jetstream ~msg_id:"consumer-ordered-2" subject
       "ordered-two");
  let ordered =
    expect_jetstream_ok "ordered create"
      (Nats_eio.Jetstream.Consumer.Ordered.v ~sw ~batch:1 ~filter_subject:filter
         stream)
  in
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      if not (Int64.equal (Nats_eio.Jetstream.Msg.consumer_sequence first) 1L)
      then failf "ordered first consumer sequence was not one";
      if not (Int64.equal (Nats_eio.Jetstream.Msg.consumer_sequence second) 2L)
      ...
      for _ = 1 to 20 do
        if not !waiting then
          ...
          in
          if
            Int.compare (Nats_eio.Jetstream.Consumer.Info.num_waiting info) 0
            > 0
          then waiting := true
      ...
      if
        not
          (Int64.equal (Nats_eio.Jetstream.Msg.consumer_sequence recovered) 1L)
      ...
      closed := true;
      match Nats_eio.Jetstream.Consumer.Ordered.next ordered with
