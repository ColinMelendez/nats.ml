...
cleanup() {
  ...
  artifact_save_text "$status" run.txt \
    ...
    "cluster_port=$cluster_base_port" "status=$status"
  if [ -n "$primary" ]; then
...
make_config() {
  ...
  sed "s/server_name: system-account/server_name: $name/" \
    "$script_dir/nats-server-system.conf" >"$target"
}

make_config cluster-a "$config_dir/cluster-a.conf"
make_config cluster-b "$config_dir/cluster-b.conf"
make_config cluster-c "$config_dir/cluster-c.conf"
...
start_cluster() {
  ...
  if ! primary=$(docker run --detach --name "$primary_name" \
    ...
    --volume "$config_dir/cluster-a.conf:/etc/nats/nats.conf:ro" \
    --publish "127.0.0.1:$cluster_base_port:4222" "$image" \
    --config /etc/nats/nats.conf -p 4222 -n cluster-a \
  ...
  if ! secondary=$(docker run --detach --name "$secondary_name" \
    ...
    --volume "$config_dir/cluster-b.conf:/etc/nats/nats.conf:ro" \
    --publish "127.0.0.1:$secondary_port:4222" "$image" \
    --config /etc/nats/nats.conf -p 4222 -n cluster-b \
    -cluster nats://0.0.0.0:6222 \
  ...
  if ! tertiary=$(docker run --detach --name "$tertiary_name" \
    ...
    --volume "$config_dir/cluster-c.conf:/etc/nats/nats.conf:ro" \
    --publish "127.0.0.1:$tertiary_port:4222" "$image" \
    --config /etc/nats/nats.conf -p 4222 -n cluster-c \
    -cluster nats://0.0.0.0:6222 \
...
status=0
if NATS_TEST_SERVERS="nats://127.0.0.1:$cluster_base_port,nats://127.0.0.1:$secondary_port,nats://127.0.0.1:$tertiary_port" \
    NATS_TEST_EXPECTED_SERVERS=3 \
    NATS_TEST_SYSTEM_USER=sys NATS_TEST_SYSTEM_PASS=sys \
    NATS_TEST_SYSTEM_SECONDARY_SERVER="nats://127.0.0.1:$secondary_port" \
    NATS_TEST_SYSTEM_RECOVERED_SERVER="nats://127.0.0.1:$tertiary_port" \
