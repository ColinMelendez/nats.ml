...
modes=${NATS_INTEROP_JETSTREAM_CLUSTER_MATRIX_MODES:-node-a,node-b,node-c,leader,restart,multi-node}

case "$modes" in
  ""|,*|*,|*,,*)
    echo "NATS_INTEROP_JETSTREAM_CLUSTER_MATRIX_MODES must contain node-a, node-b, node-c, leader, restart, and/or multi-node (seed is an alias for node-a) with no empty entries" >&2
    exit 1
    ;;
esac
...
for mode do
  case "$mode" in
    seed|node-a|node-b|node-c|leader|restart|multi-node)
      ;;
    *)
      echo "unknown JetStream cluster interop matrix mode: $mode (expected node-a, node-b, node-c, leader, restart, or multi-node)" >&2
      exit 1
      ;;
  esac
