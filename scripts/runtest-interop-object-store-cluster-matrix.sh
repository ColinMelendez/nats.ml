...
modes=${NATS_INTEROP_OBJECT_STORE_CLUSTER_MODES:-seed,leader,restart}

case "$modes" in
  ""|,*|*,|*,,*)
    echo "NATS_INTEROP_OBJECT_STORE_CLUSTER_MODES must contain seed, leader, and/or restart with no empty entries" >&2
    exit 1
    ;;
esac
...
for mode do
  case "$mode" in
    seed|leader|restart)
      ;;
    *)
      echo "unknown Object Store cluster interop mode: $mode (expected seed, leader, or restart)" >&2
      exit 1
      ;;
  esac
