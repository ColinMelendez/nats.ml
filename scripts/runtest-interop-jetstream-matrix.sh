...
case "$scenarios" in
  ""|,*|*,|*,,*)
    echo "NATS_INTEROP_JETSTREAM_MATRIX_SCENARIOS must contain pull, push, and/or ordered with no empty entries" >&2
    exit 1
    ;;
esac
...
for scenario do
  case "$scenario" in
    pull|push|ordered)
      ;;
    *)
      echo "unknown JetStream interop matrix scenario: $scenario (expected pull, push, or ordered)" >&2
      exit 1
      ;;
  esac
