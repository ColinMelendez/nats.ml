...
| Key-Value | Covered: CRUD, CAS, history, ordinary and ordered watches, listers, managers, policy fields, composition, TTL, purge-marker cleanup, and an authenticated cluster-matrix runner | Ordinary and ordered watch behavior remain separate contracts; the full authenticated cluster evidence sweep remains a release gate after cumulative server resource pressure interrupted one sequential run |
